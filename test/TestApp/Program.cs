...
namespace TestApp
{
	internal class Program
	{
		private static async Task Main()
		{
			...
			IKeyValueRepository client = factory.GetKeyValueRepository();

			var userId1 = Guid.NewGuid();

			Console.WriteLine($"{Environment.NewLine}Put new values to user {userId1}");
			CommonResponse putResult1 = await client.Put(new ItemsPutRequest
			{
				UserId = userId1,
				Items = new[]
				{
					new KeyValueModel {Key = "key1", Value = "value1"},
					new KeyValueModel {Key = "key2", Value = "value2"},
					new KeyValueModel {Key = "key3", Value = "value3"}
				}
			});
			LogData(putResult1);
			...
			CommonResponse putResult2 = await client.Put(new ItemsPutRequest
			{
				UserId = userId2,
				Items = new[]
				{
					new KeyValueModel {Key = "key1", Value = "value4"},
					new KeyValueModel {Key = "key2", Value = "value5"},
					new KeyValueModel {Key = "key3", Value = "value6"}
				}
			});
			LogData(putResult2);
			...
			ItemsResponse getResult1 = await client.Get(new ItemsGetRequest {UserId = userId1, Keys = keys1});
			LogData(getResult1);
			...
			ItemsResponse getResult2 = await client.Get(new ItemsGetRequest {UserId = userId2, Keys = keys2});
			LogData(getResult2);
			...
			CommonResponse deletetResult = await client.Delete(new ItemsDeleteRequest {UserId = userId1, Keys = keys3});
			...
			ItemsResponse getResult3 = await client.Get(new ItemsGetRequest {UserId = userId1, Keys = keys4});
			LogData(getResult3);

			Console.WriteLine($"{Environment.NewLine}Change values for user {userId2}, keys {JsonSerializer.Serialize(keys2)}");
			CommonResponse putResult3 = await client.Put(new ItemsPutRequest
			{
				UserId = userId2,
				Items = new[]
				{
					new KeyValueModel {Key = "key2", Value = "new_value4"},
					new KeyValueModel {Key = "key3", Value = "new_value5"}
				}
			});
			LogData(putResult3);

			Console.WriteLine($"{Environment.NewLine}Get values for user {userId2}, keys {JsonSerializer.Serialize(keys2)}");
			ItemsResponse getResult4 = await client.Get(new ItemsGetRequest {UserId = userId2, Keys = keys2});
			LogData(getResult4);
